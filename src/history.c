...
static gint compare_chat_history_entries(gconstpointer a, gconstpointer b,
        gpointer user_data) {
    (void)user_data;
...
static void load_chat_from_file(const char* history_path,
    const char* filename, GSList *chat_entries) {
...
static void update_chat_list(GSList *l, GListStore *history_store) {
    ChatHistoryEntry *entry = l->data;
    GtkStringObject *str_obj = gtk_string_object_new(entry->filename);
    g_list_store_append(history_store, str_obj);
...
void history_load_chats(AppData *app_data) {
    ...
    while ((filename = g_dir_read_name(dir))) {
        load_chat_from_file(history_path, filename, chat_entries);
    }
    g_dir_close(dir);
    chat_entries = g_slist_sort_with_data(chat_entries,
            compare_chat_history_entries, NULL);
    for (GSList *l = chat_entries; l != NULL; l = l->next) {
        update_chat_list(l, app_data->history_store);
    }
...
void history_save_chat(AppData *app_data) {
    ...
    const char *json_str = json_object_to_json_string_ext(
            app_data->messages_array, JSON_C_TO_STRING_PRETTY);
    g_file_set_contents(filepath, json_str, -1, NULL);
    g_free(filepath);
}

void history_start_new_chat(AppData *app_data) {
    if (app_data->current_chat_id && app_data->messages_array
            && json_object_array_length(app_data->messages_array) > 0) {
...
void history_load_selected_chat(GtkListBox *box, GtkListBoxRow *row,
    ...
    if (!row) return;
    const char *chat_id = gtk_label_get_text(GTK_LABEL(
                gtk_list_box_row_get_child(row)));
    if (app_data->current_chat_id &&
            strcmp(app_data->current_chat_id, chat_id) == 0) {
        return;
    }
    history_save_chat(app_data);
