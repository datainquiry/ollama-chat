...
A simple and lightweight GTK4 chat application for interacting with
...
- Model Selection: Easily switch between available models from a
  dropdown menu.
...
- Markdown Rendering: Assistant responses are rendered with basic
  ...
  blocks.
- Configurable: A user-editable configuration file stores your settings.
...
- From a File: Type `@` followed by the path to a local file (e.g.,
  `@/path/to/your/file.txt`). The application will read the file's
  content and include it in the prompt. If the file is binary, its
...
- Top P: Sets a threshold for nucleus sampling. The model will only
  consider the most likely tokens with a cumulative probability of
  `top_p`.
...
- Seed: A number used to initialize the model's random number generator.
  Setting a seed ensures that you get the same response for the same
  prompt every time. A value of 0 means the seed is random.
...
To install the application system-wide (including the `.desktop` file
and icon), run:
...
`~/.config/ollama-chat/config.json`. This file is created automatically
on the first run and can be edited manually. You can also change most
